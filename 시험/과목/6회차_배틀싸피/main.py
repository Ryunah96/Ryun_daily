...
def bfs(grid, start, target, wall):
    ...
    while queue:
        ...
        for d, (dr, dc) in enumerate(DIRS):
            nr, nc = r + dr, c + dc
            if (nr, nc) == target and grid[nr][nc] != 'T':
        ...
        for d, (dr, dc) in enumerate(DIRS):
            ...
            if 0 <= nr < rows and 0 <= nc < cols and (nr, nc) not in visited and grid[nr][nc] != 'W' and grid[nr][nc] != 'R' and grid[nr][nc] != 'F' and grid[nr][nc] != 'T':
                visited.add((nr, nc))
                queue.append(((nr, nc), actions + [MOVE_CMDS[d]]))

            if 0 <= nr < rows and 0 <= nc < cols and (nr, nc) not in visited and grid[nr][nc] == 'T':
                visited.add((nr, nc))
                queue.append(((nr, nc), actions + [FIRE_M_CMDS[d]] + [MOVE_CMDS[d]]))
...
FIRE_M_CMDS = {0: "R F M", 1: "D F M", 2: "L F M", 3: "U F M"}
START_SYMBOL = 'M'
TARGET_SYMBOL = 'X'
WALL_SYMBOL = 'R'
